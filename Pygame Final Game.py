...
purchase_rect_event = pygame.Rect(0, 0, 10000, 10000)
...
while True:
    ...
    if 0 <= stars_per_second < 0.6:
        stars = round(stars, 3)
    elif 0.6 <= stars_per_second < 6:
        stars = round(stars, 2)
    else:
        stars = round(stars, 1)
    for event in pygame.event.get():
        ...
        # Defining Star Fonts
        producing = star_count_font.render(str(round(stars_per_second, 3)) + " stars per second", False, white)
    ...
    for i in range(len(star_producers)):
        ...
        if purchase_rect_list[i].collidepoint(pygame.mouse.get_pos()) and pygame.mouse.get_pressed()[0] and stars >= cost_list[i]-0.0001:
            ...
            if mouse_click_counter >= 7:
                ...
                cost_list[i] *= 1.1499999  # The cost goes up by 15%
                amount_list[i] += 1  # The amount of that unit goes up by 1
                pygame.draw.rect(screen, green, purchase_rect_event)
        ...
        if purchase_rect_list[i].collidepoint(pygame.mouse.get_pos()) and pygame.mouse.get_pressed()[0] and stars < cost_list[i]:
            ...
            if mouse_click_counter_2 == 10:
                purchase_fail()
                pygame.draw.rect(screen, (255, 0, 0), purchase_rect_event)
